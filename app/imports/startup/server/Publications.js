...
Meteor.publish(Communities.userPublicationName, function () {
  if (this.userId) {
    const username = Meteor.users.findOne(this.userId).username;
    return Communities.collection.find({ owner: username });
  }
  return this.ready();
});
...
// Admin-level publication.
// If logged in and with admin role, then publish all documents from all users. Otherwise publish nothing.
Meteor.publish(Communities.adminPublicationName, function () {
  if (this.userId && Roles.userIsInRole(this.userId, 'admin')) {
    return Communities.collection.find();
  }
  return this.ready();
});

Meteor.publish(Communities.allPublicationName, function () {
  if (this.userId) {
    return Communities.collection.find();
  }
  return this.ready();
